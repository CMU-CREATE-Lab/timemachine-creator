...
def ct_read_file(path)
  open(path, 'r') { |fh| fh.read }
end

def ct_write_file(path, data)
...
class VideosetCompiler
  ...
  def write_json
    ct_mkdir_p "#{@parent.videosets_dir}/#{id}"
    dest = "#{@parent.videosets_dir}/#{id}/r.json"
    ct_write_file(dest, JSON.pretty_generate(info))
...
class Compiler
  ...
  def initialize_source(source_info)
    ...
    ct_write_file('autogenerated_framenames.txt', @source.framenames.join("\n"))
  end

  def initialize_tilestack
    ct_mkdir_p @raw_tilestack_dir
    ct_mkdir_p @tilestack_dir
    r = {'width' => @source.width, 'height' => @source.height,
         'tile_width' => @source.tilesize, 'tile_height' => @source.tilesize}
    ct_write_file("#{@tilestack_dir}/r.json", JSON.pretty_generate(r))
  end
  ...
  def initialize_destination(destination_info)
    if destination_info.class == String
      ...
      if ! File.exists? @videosets_parent_dir
        ...
        when "local"
          ct_mkdir_p @videosets_parent_dir
        when "symlink"
