...
ExifView::ExifView(std::string const& filename) {
  if (!m_data.import_data(filename)) {
    throw(ExifErr() << "Could not parse EXIF data out of \"" << filename << "\".");
  }
}

// Query the data by tag ID (common tags are enumerated at the top if
// Exif.h)
void ExifView::query_by_tag(ExifTag tag, int32& value) const {
  bool success = m_data.get_tag_value(tag, value);
  if (!success) throw(ExifErr() << "Could not read EXIF tag " << tag << ".");
}

// Query the data by tag ID (common tags are enumerated at the top if
// Exif.h)
void ExifView::query_by_tag(ExifTag tag, double& value) const {
  bool success = m_data.get_tag_value(tag, value);
  if (!success) throw(ExifErr() << "Could not read EXIF tag: " << tag << ".");
}
...
// Query the data by tag ID (common tags are enumerated at the top if
// Exif.h)
void ExifView::query_by_tag(ExifTag tag, std::string& value) const {
  bool success = m_data.get_tag_value(tag, value);
  if (!success) throw(ExifErr() << "Could not read EXIF tag: " << tag << ".");
}
...
// Query the data by tag ID (common tags are enumerated at the top if
// Exif.h)
void ExifView::query_by_tag(ExifTag tag, ExifDateTime& value) const {
...
// Camera info
std::string ExifView::get_make() const {
  std::string make;
  query_by_tag(EXIF_Make, make);
  return make;
}

std::string ExifView::get_model() const {
  std::string model;
...
ExifDateTime ExifView::get_modification_time() const {
  ExifDateTime date_time;
  query_by_tag(EXIF_DateTime, date_time);
  return date_time;
}

// Get date and time of image capture from EXIF
ExifDateTime ExifView::get_capture_time() const {
  ExifDateTime date_time;
...
ExifDateTime ExifView::get_digitization_time() const {
  ExifDateTime date_time;
...
// Camera settings
double ExifView::get_f_number() const {
  double value;
  try {
...
double ExifView::get_exposure_time() const {
  double value;
  try {
...
double ExifView::get_focal_length_35mm_equiv() const {
  double value;
  try {
  ...
  if (focal_plane_x_resolution <= 0) throw(ExifErr() << "Illegal value for FocalPlaneXResolution");
  query_by_tag(EXIF_FocalPlaneYResolution, focal_plane_y_resolution);
  if (focal_plane_y_resolution <= 0) throw(ExifErr() << "Illegal value for FocalPlaneYResolution");
  ...
  case 3: // cm
    ...
    break;
  default:
    throw(ExifErr() << "Illegal value for FocalPlaneResolutionUnit");
  }
  ...
  if (sensor_diagonal_in_mm == 0) throw(ExifErr() << "Illegal value while computing 35mm equiv focal length");
  return focal_length * sqrt(36.*36.+24.*24.) / sensor_diagonal_in_mm;
}

//// Returns the x & y focal lengths in pixels (good for working with camera)
//Vector2 ExifView::get_focal_length_pix() const {
//  double focal_length; // mm
//  Vector2 focal_plane_res;
//  query_by_tag(EXIF_FocalLength, focal_length);
...
//    throw(ExifErr() << "Illegal value for FocalPlaneXResolution");
//  if ( focal_plane_res[1] <= 0 )
//    throw(ExifErr() << "Illegal value for FocalPlaneYResolution");
//  int32 focal_plane_resolution_unit = 2;
//  try { query_by_tag(EXIF_FocalPlaneResolutionUnit, focal_plane_resolution_unit); } catch (const ExifErr&) {}
//  double focal_plane_resolution_unit_in_mm = 0;
//  switch (focal_plane_resolution_unit) {
...
//Vector2i ExifView::get_image_size() const {
//  Vector2i image_size;
...
double ExifView::get_aperture_value() const {
  double value;
  try {
    query_by_tag(EXIF_ApertureValue, value);
    return value;
...
double ExifView::get_time_value() const {
  double value;
  try {
    query_by_tag(EXIF_ShutterSpeedValue, value);
    return value;
...
double ExifView::get_exposure_value() const {
...
//
double ExifView::get_film_speed_value() const {
...
double ExifView::get_luminance_value() const {
  double Bv;
  try{
    ...
    try {
      ...
      throw(ExifErr() << "Insufficient EXIF information to compute brightness value.");
...
// http://en.wikipedia.org/wiki/Light_meter#Exposure_meter_calibration
//
double ExifView::get_average_luminance() const {
  // const double N = 1/3.125;
  const double K = 12.5;
  ...
  try {
    ...
    throw(ExifErr() << "Insufficient EXIF information to compute average scene luminance.");
    return 0; // never reached
  }
}

size_t ExifView::get_thumbnail_location() const {
  int32 offset;
  // query_by_tag throws if tag isn't found
  query_by_tag(EXIF_ThumbnailOffset, offset);
