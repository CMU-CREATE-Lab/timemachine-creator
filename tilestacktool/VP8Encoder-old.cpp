...
VP8Encoder::VP8Encoder(std::string dest_filename, int width, int height, double fps, double cqLevel) :
  total_written(0), dest_filename(dest_filename), width(width), height(height), fps(fps), cqLevel(cqLevel)  {

  tmp_filename = temporary_path(filename_sans_directory(dest_filename));
  int nthreads = 8;
  targetBitsPerPixel = 2; // larger numbers increases overall video quality
...
void VP8Encoder::close() {
  ...
  std::string cmdcommon = string_printf("\"%s\" %s.yuv -o %s -w %d -h %d --fps=%ld/1 -p 2 --codec=vp8 --fpf=%s.fpf --cpu-used=0 --target-bitrate=%ld --auto-alt-ref=1 -v --min-q=4 --max-q=52 --drop-frame=0 --lag-in-frames=25 --kf-min-dist=12 --kf-max-dist=12 --static-thresh=0 --tune=psnr --threads=%d", path_to_vpxenc().c_str(), tmp_filename.c_str(), dest_filename.c_str(), width, height, lround(fps), tmp_filename.c_str(), lround(width * height * fps * targetBitsPerPixel / 8000.0), nthreads);

  std::string pass1 = cmdcommon + string_printf(" --pass=1 --minsection-pct=0 --maxsection-pct=800 -t 0");
  std::string pass2 = cmdcommon + string_printf(" --pass=2 --minsection-pct=5 --maxsection-pct=1000 --bias-pct=50 -t 6 --end-usage=cq --cq-level=%ld --good --profile=0 --arnr-maxframes=7 --arnr-strength=5 --arnr-type=3 --psnr", lround(cqLevel));
